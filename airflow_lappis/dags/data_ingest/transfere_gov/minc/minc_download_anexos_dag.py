...
def download_anexos_dag():
    ...
    @task
    def baixar_e_salvar_anexos(lista_pendentes: list):
        """
        Recebe a lista de pendentes (com id e bucket destino), faz o request na API, 
        decodifica o base64, salva no MinIO no bucket certo e atualiza o banco.
        """
        if not lista_pendentes:
            logging.info("Nenhum anexo pendente para download.")
            return
        ...
        # Usamos um set para rastrear os buckets que já verificamos/criamos
        buckets_verificados = set()

        for pendente in lista_pendentes:
            anexo_id = pendente["id"]
            bucket_name = pendente["bucket"]
            
            # Verifica e cria o bucket de forma dinâmica caso ainda não o tenha feito nesta execução
            if bucket_name not in buckets_verificados:
                try:
                    if not s3_hook.check_for_bucket(bucket_name):
                        s3_hook.create_bucket(bucket_name=bucket_name)
                    buckets_verificados.add(bucket_name)
            ...
            try:
                ...
                logging.error(f"Erro ao baixar ou processar o anexo {anexo_id}: {e}")
                
            # Time sleep pra não tomar block da API
            time.sleep(1.5)

    # Fluxo da DAG
    lista_pendentes = buscar_ids_pendentes()
    baixar_e_salvar_anexos(lista_pendentes)
