...
-- Bronze — grupos e coletivos da Lei Paulo Gustavo, identificados pelo CPF do representante.
--
...
WITH base AS (
    SELECT
        LOWER(TRIM("nº do cpf do representante do grupo/coletivo")) AS documento_bruto,
    ...
    WHERE tabela_origem = 'lpg_dados_coletivos'
      AND "nº do cpf do representante do grupo/coletivo" IS NOT NULL
      AND LOWER(TRIM("nº do cpf do representante do grupo/coletivo")) NOT IN ('', 'nan', 'none')
),
